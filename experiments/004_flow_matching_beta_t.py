"""Flow-matching action-chunk policy — beta-weighted timestep schedule.

Fork of ``001_flow_matching_baseline.py``. Identical model, integrator, eval, and
train loop; the ONLY change is the training-time timestep distribution in
``flow_loss``: instead of t ~ U[0,1], draw t ~ Beta(a, b) with a < b so density
piles up near t≈0 (the noisy end of the probability path). The hypothesis is that
oversampling the high-noise regime — where the velocity field is hardest and the
sample-space error concentrates — sharpens the learned field there without
hurting the near-clean end. Inference (``integrate_chunk``) is unchanged; the
schedule only reweights which timesteps the loss sees.
...
The uniform-t baseline lives in ``001_flow_matching_baseline.py``; the
real-time-chunking variant — clean prefix conditioning + per-token timesteps —
...
Run:
    python experiments/004_flow_matching_beta_t.py                     # train
    python experiments/004_flow_matching_beta_t.py --eval <ckpt>       # eval a checkpoint
    python experiments/004_flow_matching_beta_t.py --cfg.t-beta-a 1 --cfg.t-beta-b 5   # heavier noise weighting
"""
...
class TrainConfig:
    ...
    L_ctx: int = 256
    L_chunk: int = 16
    # training-time timestep schedule: t ~ Beta(t_beta_a, t_beta_b). a < b weights
    # density toward t≈0 (noisy end of the path); a == b == 1 recovers U[0,1] (the
    # 001 baseline). Default Beta(1, 3) has density 3(1-t)^2 — monotone-decreasing,
    # mean 0.25 — so the loss oversamples the high-noise regime.
    t_beta_a: float = 1.0
    t_beta_b: float = 3.0
    # inference
    n_flow_steps: int = 8
...
def _model_tag(cfg: TrainConfig) -> str:
    return (
        f"fmbeta-d{cfg.d_model}-L{cfg.n_layers}-H{cfg.n_heads}-Lc{cfg.L_ctx}-Lk{cfg.L_chunk}"
        f"-fs{cfg.n_flow_steps}-b{cfg.t_beta_a:g}_{cfg.t_beta_b:g}"
    )
...
class FlowMatchingPolicy(nn.Module):
    ...
    def __init__(self, cfg: TrainConfig):
        ...
        self.t_beta_a = cfg.t_beta_a
        self.t_beta_b = cfg.t_beta_b
        d = cfg.d_model
...
# %%
def sample_beta_t(
    B: int, a: float, b: float, *, device: torch.device, gen: torch.Generator | None = None
) -> Float[Tensor, " B"]:
    """``B`` timesteps t ~ Beta(a, b) in [0, 1]. ``a == b == 1`` gives U[0,1].

    ``torch.distributions.Beta`` can't take a generator, so reproducibility (the
    fixed-noise val metric) is threaded through ``gen`` by drawing a seed from it
    and forking the global RNG around the Beta draw — leaving the caller's ``gen``
    advanced deterministically. With ``gen=None`` (training) it's a plain stochastic
    draw. The Beta params are CPU scalars, so the draw is on CPU; only the CPU RNG
    is forked before moving the result to ``device``."""
...
def flow_loss(
    ...
    Draws a timestep t ~ Beta(model.t_beta_a, model.t_beta_b) (noise-weighted, vs the
    001 baseline's U[0,1]) and noise z ~ N(0,I), interpolates a_t between z and the
    ground-truth chunk, and returns the element-wise squared error v̂ vs the target
    velocity. ``.mean()`` it for the training scalar; slice + ``.mean()`` it for the
    modality/horizon decomposition (``velocity_mse_breakdown``) — same tensor, so the
    marginals exactly partition the scalar."""
    target = batch.target
    B = target.shape[0]
    t = sample_beta_t(B, model.t_beta_a, model.t_beta_b, device=target.device, gen=gen)
    ...
    *,
    ...
    wandb.init(
        ...
        tags=[
            ...
            f"d{cfg.d_model}",
            f"L{cfg.n_layers}",
            f"beta{cfg.t_beta_a:g}_{cfg.t_beta_b:g}",
        ],
        config=asdict(cfg),
    )
...
def main(args: Args) -> None:
    ...
    cfg = args.cfg
    stats = load_consolidated_stats(Path(cfg.data_root) / "stats.json")
    auto_comment = f"beta{cfg.t_beta_a:g}_{cfg.t_beta_b:g}-{cfg.max_steps // 1000}k-b{cfg.batch_size}"
